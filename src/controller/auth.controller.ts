...
import {
  ...
  loginErr,
  loginErr1,
} from "../helpers/authController/login/errObj.js";
import {
  ifEmailAlreadySent,
  ifEmailNot,
  ifUserWithEmailExist,
} from "../helpers/authController/sendOpt/errObj.js";
import {
  ifOtpNotMatch,
  ifOtpWithEmailNot,
} from "../helpers/authController/verifyOtpForSignup/errObj.js";
import {
  ifBothPwdNotEqual,
  ifUserExist,
  wrongEmailProvided,
} from "../helpers/authController/signupWithEmail/errObj.js";
import { ifGooglePayloadNot } from "../helpers/authController/signupWithGoogle/errObj.js";
import {
  ifPwdNotEqual,
  ifUserResetingPwdNotFound,
} from "../helpers/authController/verifyOtp/errObj.js";
import {
  forgetpwdIfUsername,
  forgetpwdIfUserNot,
...
import {
  ...
  ifUserExistSignin,
} from "../helpers/authController/signupWithGoogle/resObj.js";
import {
  chkOtp,
  updateUser,
} from "../helpers/authController/verifyOtp/verifyoptFunc.js";
...
import {
  ...
  sendRes,
} from "../utils/reusable/reusableFunc.js";
...
export const login = async (req: Rq, res: Rs) => {
  ...
  if (!checkuser.password) throw new AirClipErr(ifUserDidntCreatedPassword);
  else {
...
export const sendOtp = async (req: Rq, res: Rs) => {
  const { email } = req.body;

  if (!email) throw new AirClipErr(ifEmailNot);

  const user = await User.findOne({ email });

  if (user) throw new AirClipErr(ifUserWithEmailExist);

  const otp = generateOtp();

  const createdOtp = await VerifyEmailOtp.findOne({ email });

  if (createdOtp) throw new AirClipErr(ifEmailAlreadySent);
...
export const verifyOtpForSignup = async (req: Rq, res: Rs) => {
  const { email, otp } = req.body;

  const createdOtp = await VerifyEmailOtp.findOne({ email });

  if (!createdOtp) throw new AirClipErr(ifOtpWithEmailNot);

  if (createdOtp.otp !== Number(otp)) throw new AirClipErr(ifOtpNotMatch);

  sendRes(res, ifOtpMatch);
};

export const signupWithEmail = async (req: Rq, res: Rs) => {
  //done 🟢
  const { email, password, confirmPassword } = req.body;

  if (password !== confirmPassword) throw new AirClipErr(ifBothPwdNotEqual);

  const exist = await User.findOne({ email });

  if (exist) throw new AirClipErr(ifUserExist);
...
export const signupWithGoogle = async (req: Rq, res: Rs) => {
  const { token } = req.body;

  const payload = await getPayloadFromGoogle(token);

  if (!payload || !payload.email) throw new AirClipErr(ifGooglePayloadNot);
...
export const forgetpassword = async (req: Rq, res: Rs) => {
  ...
  if (!user) throw new AirClipErr(forgetpwdIfUserNot);
...
export const verifyOtp = async (req: Rq, res: Rs) => {
  // done🟢
  const { otp, newPassword, confirmPassword, email } = req.body;

  if (newPassword !== confirmPassword) throw new AirClipErr(ifPwdNotEqual);
  ...
  if (!updatedUser) throw new AirClipErr(ifUserResetingPwdNotFound);
